<!-- CHECKPOINT — disaster-recovery snapshot, 20260831T191540
     State: Skill thread (#3) fully closed — catalog, advancement (cap-based), prerequisites, categories, custom skills.
...
- **Role (hypothesized concept):** a named collection of tags as held together in practice — "Director" is a prime candidate. Whether a Role is a live object (editing it changes every holder) or a grant-time copy of tags is **parked for the Permissions session**.
...
- **Policies are values. Permissions (§15) are who-can-do-what.** They are separate concepts, discussed separately. Whether Permissions internally reuse the policy model is **parked**.
...
- **Declared exception: the speculative character cap gates creation only** — existing over-cap state is never flagged (§6).
...
| P8 | Speculative character cap | number | players 3 / staff 10 | Per-role default | Declared exception: creation-gating only. §6 |
...
- **Working → Committed is gated by its own dedicated tag** (one-tag-one-thing, §15); likely rolled into a future Role. Mechanics parked for the Permissions session.
