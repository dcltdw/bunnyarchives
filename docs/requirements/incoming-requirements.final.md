<!-- CHECKPOINT — disaster-recovery snapshot, 20260901T045556
     State: all registry defaults resolved; P22 = item print formats; class/tier optional attributes. Elicitation list complete.
...
- **At install there are zero root holders.** Until the configured minimum is reached, a **Bootstrap Lock** is in effect:
  ...
  - Bootstrap Lock lifts once the configured minimum is met (e.g., at 3 holders under a `min 3` config) **and at least one required waiver is defined (P5)**. The wizard also sets the current rulebook edition (P19).
...
- **Item print format is a policy vocabulary (P22)** — physical shape, eight formats seeded from card to page; print layout defaults per format and is overridable per item (three-layer pattern, §15).
- **Class and tier are first-class attributes on items — and always optional.** Every item may sit in one giant untyped bucket; class/tier exist for those who want them, and are distinct from print format.
...
| P2 | Season pass duration | event count *or* end date | 4 events | Global | No grandfathering (R10). §2 |
| P3 | Community Fund approval mode | enum: managers-approve / self-select | managers-approve | Global | §2 |
| P4 | Fund balance visibility | enum: everyone / staff only / managers only | managers only | Global | §2 |
| P5 | Required waivers | list | empty; **bootstrap wizard requires ≥1 waiver before Bootstrap Lock lifts** | Global | Checked at each registration (current-state rule). §5, §15 |
...
| P9 | Registration approval required | boolean | off | Global | §11 |
...
| P15 | Default on-deck alert list | list of (offset, label) | T-40 "reminder — eat something"; T-20 "head to the module building" | Global + per-activity override | Offsets relative to T=0; recipients default to all assigned. §16 |
...
| P19 | Current rulebook edition | identifier | none — **set in the bootstrap wizard** | Global | §1, §15 |
...
| P22 | Item print format | list of (name, dimensions) | small card 4.25×2.5; wide small card 6×2.5; short slip 8.5×2.5; tall card 4.25×5; wide tall card 6×5; tall slip 8.5×5; page 8.5×11; half-page 8.5×5.5 (inches) | Global | Print layout defaults per format, per-item override. §19 |
